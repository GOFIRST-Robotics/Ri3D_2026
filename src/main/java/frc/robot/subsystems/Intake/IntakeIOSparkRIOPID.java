...
public class IntakeIOSparkRIOPID implements IntakeIO {
    ...
    public IntakeIOSparkRIOPID() { 
        ...
        doorConfig.closedLoop.feedForward
                .kS(0.0)
                .kV(IntakeConstants.INTAKE_WHEEL_MOTOR_ID)
                .kA(0.0);
    ...
    public void setIntakeDoorPosition(double position, IntakeIOInputs inputs) {
        ...
        targetPosition = MathUtil.clamp(position, MIN_DOOR_POSITION, MAX_DOOR_POSITION);
        
        if (position != targetPosition) {
            System.out.println("SAFETY: Clamped intake position from " + position + " to " + targetPosition);
        }

        // Control the LEADER motor (leftDoorMotor) using standard position PID
        if(Math.abs(targetPosition-IntakeConstants.INTAKE_DOOR_POSITION_DEPLOYED)<.01){
            DOWN=true;
        } else {
            DOWN=false;
        }
        
        System.out.println("motor set to: " +targetPosition);
    }
    ...
    @Override
        ...
        double positionError = ((targetPosition - inputs.currentDoorPosition + Math.PI) % (2*Math.PI) + 2*Math.PI) % (2*Math.PI) - Math.PI;
        // double positionError = targetPosition - (inputs.currentDoorPosition%Math.PI*2);
        positionError = Math.abs(positionError) > .025 ?  positionError : 0;

        double velocitySetpoint = IntakeConstants.VELOCITY_GAIN * positionError;          // scale error to velocity
        velocitySetpoint = Math.max(-IntakeConstants.MAX_INTAKE_PIVIOT_VELO, Math.min(IntakeConstants.MAX_INTAKE_PIVIOT_VELO, velocitySetpoint)); // clamp
        leftDoorMotor.getClosedLoopController().setSetpoint(velocitySetpoint, ControlType.kVelocity); 
        ...
        SmartDashboard.putNumber("velo setpoint", velocitySetpoint);
